...
function setTagsStyle(this:void, dia:IDialogueToRender, options:ISetTagsStyleOptions) {
  ...
  for (let len = dia.parsedText.content.length, i = 0; i < len; ++i) {
    ...
    for (let lenj = parts.length, j = 0; j < lenj; j++) {
      ...
      if (t.p) {
        cn.appendChild(createDrawing(cn, ct, dia));
        if (t.pbo) {
...
function setDialoguePosition(this:void, dia:IDialogueToRender) {
    const effect:IEffect = dia.Effect as IEffect;
    if (effect) {
        if (effect.name === "banner") {
            if (dia.Alignment <= 3) {
                dia.y = this.height - dia.height - dia.MarginV;
            }
        ...
        if (/^scroll/.test(effect.name)) {
            dia.y = /up/.test(effect.name) ? this.height : -dia.height;
            if (dia.Alignment % 3 === 1) {
                dia.x = 0;
            }
            if (dia.Alignment % 3 === 2) {
                dia.x = (this.width - dia.width) / 2;
            }
            if (dia.Alignment % 3 === 0) {
                dia.x = this.width - dia.width;
            }
        }
    } else {
        if (dia.pos) {
            if (dia.Alignment % 3 === 1) {
                dia.x = this.scale * dia.pos.x;
            }
            if (dia.Alignment % 3 === 2) {
                dia.x = this.scale * dia.pos.x - dia.width / 2;
            }
            if (dia.Alignment % 3 === 0) {
                dia.x = this.scale * dia.pos.x - dia.width;
            }
            if (dia.Alignment <= 3) {
                dia.y = this.scale * dia.pos.y - dia.height;
            }
            if (dia.Alignment >= 4 && dia.Alignment <= 6) {
                dia.y = this.scale * dia.pos.y - dia.height / 2;
            }
            if (dia.Alignment >= 7) {
                dia.y = this.scale * dia.pos.y;
            }
        } else {
            if (dia.Alignment % 3 === 1) {
                dia.x = 0;
...
function setDialogueStyle(this:void, dia:IDialogueToRender) {
    ...
    if (!dia.Effect) {
        const mw = this.width - this.scale * (dia.MarginL + dia.MarginR);
...
function setTransformOrigin(dia:IDialogueToRender) {
  ...
  for (let i = children.length - 1; i >= 0; i--) {
    if (children[i].dataset.hasRotate) {
      ...
      const tox = dia.org.x - dia.x - children[i].offsetLeft;
      const toy = dia.org.y - dia.y - children[i].offsetTop;
...
function setClipPath(this:void, dia:IDialogueToRender, options:ISetClipPathOptions):void {
  if (dia.clip) {
    ...
    fobb.style.cssText += createClipPath(dia, {
        tree:options.tree
    });
  }
}
